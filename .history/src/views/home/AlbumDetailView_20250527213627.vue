    ...
    <div class="content-wrapper">
      ...
      >
        <el-form :model="photoMetaForm" label-width="80px">
          ...
          <el-form-item label="主题">
            <el-input v-model="photoMetaForm.title" placeholder="输入主题" />
          </el-form-item>
        </el-form>
        <template #footer>
  ...
  methods: {
    ...
    async confirmPhotoMeta() {
      ...
      try {
        ...
        this.$message.error("上传处理出错，请稍后重试");
      } finally {
...
async deleteSelectedPhotos() {
  ...
  try {
    for (const photo of this.selectedPhotos) {
      await apiClient.delete(`/photos/${photo.photoId}`);
      this.album.photos = this.album.photos.filter(p => p.photoId !== photo.photoId);
      this.album.photoCount--;
    }
    this.selectedPhotos = [];
