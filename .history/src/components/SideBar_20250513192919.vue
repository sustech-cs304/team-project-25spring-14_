<template>
  ...
  <nav class="sidebar">
    ...
    <ul class="nav-list">
      <li
        ...
        :key="item.name"
        class="nav-item"
        :class="{ active: activeNav === item.name, disabled: isItemDisabled(item) }"
  ...
  <el-dialog v-model="showAppealDialog" title="账户申诉" width="400px">
    <p>您的账户已被封禁，请完成整改后点击下方按钮提交申诉请求，管理员将进行审核。</p>
    <template #footer>
...
export default {
  ...
  methods: {
    isItemDisabled(item) {
      return item.name === 'share' && this.userInfo.status === 'disabled';
    },
    handleNavClick(item) {
