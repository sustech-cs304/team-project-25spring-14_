...
async function showCard(userId, position) {
  ...
  try {
    ...
    if (String(userId) === cuurentUserId) {
      isSelf.value = true;
    } else {
      isSelf.value = false;
    }
...
function goToSpace() {
  router.push({
    path: "/share",
    query: { spaceOwnerId: OwnerId.value },
  });
}
...
async function confirmEdit() {
  try {
    const response = await apiClient.put("/user/update", {
      userId: JSON.stringify(OwnerId.value),
      username: JSON.stringify(usename.value),
      nickname: JSON.stringify(nickname.value),
      email: JSON.stringify(email.value),
    });
    if (response.data.code === 0) {
      alert("信息修改成功！");
    } else {
      alert(`信息修改失败：${response.data.message}`);
    }
  } catch (error) {
