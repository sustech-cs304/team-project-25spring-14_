  ...
  computed: {
    filterPhotos() {
      ...
      return this.photos.filter((photo) => {
        if (startDate && photo.capturedAt < startDate) return false;
        if (endDate && photo.capturedAt > endDate + 1) return false;
        if (tag && !photo.tag.includes(tag)) return false;
        if (location && !photo.location.includes(location)) return false;
