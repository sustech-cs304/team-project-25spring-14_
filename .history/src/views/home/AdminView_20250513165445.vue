<template>
  ...
  <el-dialog v-model="reportDialogVisible" title="举报详情" width="500px">
    <p>举报ID：{{ selectedReport.reportId }}</p>
    <p>举报人ID：{{ selectedReport.reporterId }}</p>
    <p>资源类型：{{ selectedReport.resourceType }}</p>
    <p>资源ID：{{ selectedReport.resourceId }}</p>
    <p>理由：{{ selectedReport.reason }}</p>
...
export default {
    ...
    filteredReports() {
      return this.reports.filter((report) => {
        if (this.currentReportTab === "unhandled") {
          ...
          return report.status === "reviewed" && report.isCorrect === false;
        } else if (this.currentReportTab === "rectified") {
          return report.status === "reviewed" && report.isCorrect === true;
        }
        return false;
      });
    },
  ...
  methods: {
    ...
    viewReportedResource() {
      console.log("查看资源", this.selectedReport.resourceId);
    },
    banReportedUser() {
      console.log("封禁用户", this.selectedReport.reporterId);
    },
    ignoreReport() {
