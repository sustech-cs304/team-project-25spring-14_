...
export default {
  ...
  data() {
    return {
      ...
      albumOwner_userId: null,
    };
  },
  methods: {
    async fetchAlbums() {
      if (!this.albumOwner_userId) {
      ...
      try {
        await apiClient
          .get(`/albums/user/${this.albumOwner_userId}`, {
            params: { currentUserId: this.albumOwner_userId },
          })
          .then((response) => {
    ...
    async handleAddAlbum({ albumTitle, albumDescription, privacy }) {
      ...
      try {
        ...
        const response = await apiClient.post(
          `albums/upload?userId=${this.albumOwner_userId}`,
          formData,
          {
            headers: {
  ...
  mounted() {
    this.albumOwner_userId = this.$route.query.userId || this.currentUserId;
    this.initializePage();
  },
};
</script>
