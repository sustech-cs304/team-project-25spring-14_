  ...
  methods: {
    ...
    photoFilter(photo) {
      ...
      const isWithinDateRange =
        (!startDate || photoDate >= new Date(startDate)) &&
        (!endDate || photoDate <= new Date(endDate));
      const matchesTag = !tag || photo.tag.includes(tag);
      const matchesLocation = !location || photo.location.includes(location);
      const matchesFavorite = isFavorite === null || photo.isFavorite === isFavorite;

      return (
        isWithinDateRange && matchesTag && matchesLocation && matchesFavorite
      );
    },
  },
};
