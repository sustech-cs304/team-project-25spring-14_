    ...
    <div class="content-wrapper">
      ...
      <div v-if="filterDialogVisible" class="filter-form">
        <el-form :model="filterCriteria" label-width="80px" inline>
          ...
          <el-form-item>
            <el-button size="small" @click="filterDialogVisible = false"
              >取消</el-button
            >
  ...
  computed: {
    filterPhotos() {
      ...
      return this.photos.filter((photo) => {
        ...
        if (photo.tag === null) const photoTag = "";
        else photoTag = photo.tag;
        if (startDate && date < startDate) return false;
        if (endDate && date > endDate) return false;
        if (
          tag &&
          !(photo.tag || "").toLowerCase().includes(tag.trim().toLowerCase())
        )
          return false;
        if (
          location &&
