<template>
      ...
      <div class="content-area">
        ...
        <div v-if="currentTab === 'reports'">
          <div class="report-tab-buttons">
            ...
            <button
              class="report-tab-button"
              :class="{ active: currentReportTab === 'all' }"
              @click="selectReportTab('history')"
            >
              处理历史
            </button>
          </div>
  ...
  <el-dialog v-model="reportDialogVisible" title="举报详情" width="700px">
    ...
    <p>用户是否整改：{{ selectedReport.isCorrected }}</p>
    ...
    <template #footer>
      ...
      <el-popconfirm title="确认解封该用户？" @confirm="activateReportedUser">
        <template #reference>
          <el-button
            v-if="
              currentReportTab === 'banned' || currentReportTab === 'rectified'
            "
            type="success"
          >
            解封该用户
      ...
      <el-popconfirm title="确认封禁该用户？" @confirm="banReportedUser">
        <template #reference>
          <el-button v-if="currentReportTab === 'unhandled'" type="danger">
            封禁该用户
          </el-button>
        </template>
      </el-popconfirm>
      <el-popconfirm title="确认忽略该举报？" @confirm="ignoreReport">
        <template #reference>
          <el-button v-if="currentReportTab === 'unhandled'"> 忽略 </el-button>
        </template>
      </el-popconfirm>
    </template>
  </el-dialog>
...
export default {
    ...
    filteredReports() {
      if (this.currentReportTab === "unhandled") {
        ...
        return this.reports.filter(
          (report) =>
            report.status === "reviewed" && report.isCorrected === false
        );
      } else if (this.currentReportTab === "rectified") {
        return this.reports.filter(
          (report) =>
            report.status === "reviewed" && report.isCorrected === true
        );
      } else if (this.currentReportTab === "history") {
        return this.reports.filter((report) => report.status === "finished");
      }
      return [];
    },
  },
  methods: {
    ...
    async ignoreReport() {
      ...
      const reporteeId = this.selectedReport.reporteeId;
      await apiClient.put(
        `/reports/admin/${reportId}?status=finished&reporteeId=${reporteeId}`
      );
      await apiClient.put(
        `/reports/admin/active/${this.selectedReport.reporteeId}`
      );
      this.$message.success("已忽略举报");
      this.reportDialogVisible = false;
      this.fetchReports();
    },
    async activateReportedUser() {
      const reportId = this.selectedReport.reportId;
      const reporteeId = this.selectedReport.reporteeId;
      try {
        await apiClient.put(
