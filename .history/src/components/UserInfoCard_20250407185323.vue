...
async function showCard(user, position) {
  const cardWidth = 350; // assuming the card width is 350px
  const padding = 20; // assuming some padding value

  top.value = position.bottom;
  if (position.left + cardWidth + padding <= window.innerWidth) {
    left.value = position.left - cardWidth - padding;
  } else {
    left.value = position.left + padding;
  }
