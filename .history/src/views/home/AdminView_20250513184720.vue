<template>
  ...
  <el-dialog v-model="reportDialogVisible" title="举报详情" width="700px">
    ...
    <template #footer>
      ...
      <el-button
        v-if="currentReportTab === 'banned' || currentReportTab === 'rectified'"
        type="success"
        @click="activateReportedUser"
      >
        解封该用户
      </el-button>
      <el-button v-else type="danger" @click="banReportedUser">
        封禁该用户
      </el-button>
      <el-button v-if="currentReportTab === 'unhandled'" @click="ignoreReport"
        >忽略</el-button
      >
    </template>
  </el-dialog>
  ...
  computed: {
    ...
    filteredReports() {
      if (this.currentReportTab === "unhandled") {
        ...
        return this.reports.filter(report => report.status === "reviewed" && report.isCorrected === false);
      } else if (this.currentReportTab === "rectified") {
        return this.reports.filter(report => report.status === "reviewed" && report.isCorrected === true);
      } else if (this.currentReportTab === "all") {
        return this.reports.filter(report => report.status === "reviewed");
      }
      return [];
    },
  },
