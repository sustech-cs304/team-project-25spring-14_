<template>
      ...
      <div class="photo-grid">
        ...
        >
          <template v-if="multiSelectMode">
            ...
            />
            <el-dialog v-model="memoryDialogVisible" title="生成回忆设置" width="400px">
  <el-form label-width="100px">
    <el-form-item label="转场效果">
      <el-select v-model="memoryOptions.transition" placeholder="选择转场">
  ...
  <template #footer>
    ...
    <el-button type="primary" @click="submitMemoryRequest">生成</el-button>
  </template>
</el-dialog>
          </template>
          <img
...
export default {
  ...
  methods: {
    ...
    async generateMemory() {
      console.log("生成回忆按钮被点击，选中的照片：", this.selectedPhotos);
      const selectedPhotoIds = this.selectedPhotos.map(
        (photo) => photo.photoId
      );
      console.log("选中照片的ID数组：", selectedPhotoIds);
      try {
        const response = await apiClient.post("/video/create_photo", {
          photoId: selectedPhotoIds,
          transition: "fade",
          fps: 25,
        });
        console.log("生成回忆成功：", response.data);
        this.$message.success("回忆生成成功");
      } catch (error) {
        console.error("生成回忆失败：", error);
        this.$message.error("生成回忆失败，请稍后再试");
      }
    },
    generateMemory() {
