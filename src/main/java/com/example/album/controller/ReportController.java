...
public class ReportController {
    ...
    public Result updateReportStatus(@PathVariable Integer reportId, @RequestParam String status, @RequestParam Integer resourceId) {
        Map<String, Object> claims = ThreadLocalUtil.get();
        String role = (String) claims.get("role");
        if (!"admin".equals(role)) {
            return Result.error("权限不足，只有管理员可以处理举报信息");
        ...
        reportService.disableUser(resourceId);
        return Result.success();
    }

    @PutMapping("/admin/active/{resourceId}")
    public Result activeUser(@PathVariable Integer resourceId) {
        Map<String, Object> claims = ThreadLocalUtil.get();
        String role = (String) claims.get("role");
        if (!"admin".equals(role)) {
            return Result.error("权限不足，只有管理员可以处理举报信息");
        ...
        reportService.activeUser(resourceId);
        return Result.success();
    }
