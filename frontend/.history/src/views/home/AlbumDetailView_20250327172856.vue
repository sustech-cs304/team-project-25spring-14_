      ...
      <div class="album-header">
        ...
        <div class="album-info-wrapper">
          <div class="album-title-row">
            ...
            <el-select v-model="editablePrivacy" size="small" @change="updateAlbumPrivacy">
              <el-option label="公开" value="PUBLIC">
                <img src="@/assets/images/PUBLIC.svg" alt="公开" class="privacy-icon" />
                <span style="margin-left: 6px">公开</span>
              </el-option>
              <el-option label="私密" value="PRIVATE">
                <img src="@/assets/images/PRIVATE.svg" alt="私密" class="privacy-icon" />
                <span style="margin-left: 6px">私密</span>
              </el-option>
              <el-option label="共享" value="SHARED">
                <img src="@/assets/images/SHARED.svg" alt="共享" class="privacy-icon" />
                <span style="margin-left: 6px">共享</span>
              </el-option>
            </el-select>
          </div>
  ...
  methods: {
    ...
    async updateAlbumTitle() {
      ...
      if (this.editableTitle !== this.album.title) {
        try {
          await apiClient.put(`/albums/${this.album.albumId}`, {
            title: this.editableTitle,
          });
          this.album.title = this.editableTitle;
          this.$message.success("相册标题已更新");
        } catch (err) {
    ...
    async updateAlbumDescription() {
      ...
      if (this.editableDescription !== this.album.description) {
        try {
          ...
          this.album.description = this.editableDescription;
          this.$message.success("相册描述已更新");
        } catch (err) {
    ...
    async updateAlbumPrivacy() {
      if (this.editablePrivacy !== this.album.privacy) {
        try {
          await apiClient.put(`/albums/${this.album.albumId}`, {
            privacy: this.editablePrivacy,
          });
          this.album.privacy = this.editablePrivacy;
          this.$message.success("相册权限已更新");
        } catch (err) {
          this.$message.error("更新权限失败");
        }
      }
    },
    cancelOrUpdateTitle(event) {
    ...
    cancelOrUpdateDescription(event) {
      const newText = event.target.innerText.trim();
      this.editingDescription = false;
