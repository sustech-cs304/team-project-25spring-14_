  ...
  methods: {
    ...
    async updateAlbumCover(photoId) {
      ...
      try {
        ...
        let coverPhotoId;
        await apiClient
          .post(`/albums/${this.album.albumId}/cover/upload`, formData, {
            headers: {
              "Content-Type": "multipart/form-data",
            },
          })
          .then((response) => {
            coverPhotoId = response.data.data.coverPhotoId;
          })
          .catch((error) => {
        ...
        if (coverPhotoId) {
          await this.updateAlbum({ coverPhotoId });
          const photo = this.album.photos.find((p) => p.photoId === coverPhotoId);
          if (photo) {
