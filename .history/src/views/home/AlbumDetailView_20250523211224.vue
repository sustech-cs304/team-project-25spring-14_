      ...
      <div class="album-header">
        ...
        <div class="album-info-wrapper">
          ...
          <div class="header-actions">
            <el-button size="small" @click="filterDialogVisible = !filterDialogVisible">筛选</el-button>
          </div>
        </div>
      ...
      <div v-if="filterDialogVisible" class="filter-form">
        <el-form :model="filterCriteria" label-width="80px" inline>
          <el-form-item label="开始">
            <el-date-picker v-model="filterCriteria.startDate" type="date" placeholder="开始日期" value-format="YYYY-MM-DD" />
          </el-form-item>
          <el-form-item label="结束">
            <el-date-picker v-model="filterCriteria.endDate" type="date" placeholder="结束日期" value-format="YYYY-MM-DD" />
          </el-form-item>
          ...
          <el-form-item label="收藏">
            <el-switch v-model="filterCriteria.isFavorite" :active-value="true" :inactive-value="null"/>
          </el-form-item>
        </el-form>
      </div>
      <template v-for="group in groupedPhotos" :key="group.date">
        <div class="date-separator"><span>{{ group.date }}</span></div>
        <div class="photo-grid">
          <div class="photo-card" v-for="photo in group.photos" :key="photo.photoId" @click="openPhotoViewer(photo)">
            <img v-if="!isVideo(photo.fileUrl)" :src="photo.thumbnailUrl" class="photo-image"/>
            <video v-else :src="photo.fileUrl" controls class="photo-image"/>
          </div>
          <div
            v-if="isSelf && group === groupedPhotos[0]"
...
export default {
  ...
  computed: {
    ...
    formatDate() {
      return dateTime => dateTime ? dateTime.split('T')[0] : '';
    },
    // Group filtered photos by date
    groupedPhotos() {
      const groups = {};
      this.filterPhotos.forEach(photo => {
        const date = this.formatDate(photo.capturedAt || photo.createdAt);
        (groups[date] = groups[date] || []).push(photo);
      });
      return Object.keys(groups).sort().map(date => ({ date, photos: groups[date] }));
    },
    // Apply inline filters on album.photos
    filterPhotos() {
      const { startDate, endDate, tag, location, isFavorite } = this.filterCriteria;
      return this.album.photos.filter(photo => {
        ...
        if (tag != null && tag.trim() !== "" && !((photo.tag||'').toLowerCase().includes(tag.trim().toLowerCase()))) return false;
        if (location != null && location.trim() !== "" && !((photo.location||'').toLowerCase().includes(location.trim().toLowerCase()))) return false;
...
.date-separator::after {
  content: ""; flex: 1; height: 1px; background: #d1d5db; margin: 0 8px;
}
</style>
