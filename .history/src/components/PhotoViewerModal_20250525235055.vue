...
<script>
import { apiClient } from "@/api"; // 假设你有一个 apiClient 用于发送请求
export default {
  ...
  methods: {
    ...
    async finalizeSubtitleEdit() {
      try {
        const payload = {
          subtitle_map: JSON.parse(this.finalSubtitleObject),
          ...
          video_url: this.photo.fileUrl,
        };
        const res = await apiClient.post("/video/add_caption", payload);
