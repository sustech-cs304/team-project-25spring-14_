    ...
    <div class="main">
      <div :class="['block', 'block_register', { 'is-txl': isLogin }]">
        <form @submit.prevent="register">
          ...
          <input
            ...
            :class="{ error: passwordError }"
          />
          <p v-if="passwordError" class="error-text">两次输入的密码不一致</p>
          <div class="form_button" @click="register">Register</div>
        </form>
      </div>
      <div :class="['block', 'block_login', { 'is-txl is-z200': isLogin }]">
        <form @submit.prevent="login">
          ...
          <span class="text">or use your email</span>
          <input v-model="loginForm.email" type="text" placeholder="Email" />
          <input
            v-model="loginForm.password"
            type="password"
            placeholder="Password"
          />
          <div class="form_button" @click="login">Login</div>
        </form>
      </div>
...
export default {
  ...
  methods: {
    ...
    register() {
      ...
      if (this.registerForm.password !== this.registerForm.confirmPassword && this.registerForm.password !== "") {
      ...
      axios
        .post(
          "http://127.0.0.1:4523/m1/5913862-5600854-default/user/register",
          this.registerForm
        )
        .then((res) => {
