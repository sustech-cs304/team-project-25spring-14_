...
export default {
  ...
  async created() {
    ...
    try {
      ...
      const photoRes = await apiClient.get(`/photos/album/${albumId}/cover/upload`, {
        params: {
          userId: this.userId,
        },
      });
      coverPhotoId = photoRes.data.data.coverPhotoIdphotoId;
    } catch (error) {
  ...
  methods: {
    ...
    async updateAlbumCover(photoId) {
      ...
      try {
        ...
        const response = await apiClient.post(`/albums/${this.albumId}`, formData, {
          headers: {
            "Content-Type": "multipart/form-data",
          },
        });
