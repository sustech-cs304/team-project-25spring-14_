<template>
  <div class="main-container">
    ...
    <div class="container">
      <!-- 标题部分 -->
      <div class="header">
        <h1 class="title">{{ this.ownerInfo.username }} 的相册集</h1>
      ...
      <div class="album-grid">
        ...
        <div
          v-if="isSelf"
          class="album-card add-album-card"
 ...
 * tool: ChatGpt
 * version: latest
 * usage: I learn from ai about how to build the album page with several components
 */
...
export default {
  ...
  data() {
    return {
      ...
      ownerInfo: {},
      showModal: false,
      albumOwnerId: this.$route.query.albumOwnerId || null,
      isSelf: true,
    };
  },
  methods: {
    ...
    async fetchOwnerInfo() {
      try {
        await apiClient
          .get(`/user/getUser/${this.albumOwnerId}`)
          .then((response) => {
        ...
        console.log("相册拥有者信息获取成功，ID:", this.userInfo.userId);
      } catch (error) {
        console.error("相册拥有者信息获取失败:", error);
        alert("获取相册拥有者信息失败，请重试！");
      }
    },
    async fetchAlbums() {
      // if (!this.userInfo) {
      ...
      try {
        await apiClient
          .get(`/albums/user/${this.albumOwnerId}`, {
        ...
        for (const album of this.albums) {
          if (album.coverPhotoId) {
            const response = await apiClient.get(
              `/photos/${album.coverPhotoId}`
            );
            album.coverThumbnailUrl = response.data.data.thumbnailUrl;
            album.coverPhotoUrl = response.data.data.fileUrl;
          } else {
            album.coverThumbnailUrl = new URL(
              "@/assets/images/albumlogo.svg",
              import.meta.url
            ).href;
          }
        }
    ...
    async initializePage() {
      ...
      if (this.albumOwnerId == null) {
        this.albumOwnerId = this.userInfo.userId;
      }
      await this.fetchOwnerInfo();
      await this.fetchAlbums();
      localStorage.setItem("activeNav", "albums");
      if (this.currentUserId == this.albumOwnerId) {
        this.isSelf = true;
      } else {
        this.isSelf = false;
      }
...
.default-cover {
  ...
  background-image: url("/src/assets/images/albumlogo.svg");
